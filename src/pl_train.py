...
from eval_utils import get_char_logits, my_get_results, get_predictions, get_score, create_labels_for_scoring, \
    compute_metrics
from arguments import parse_args_train_pl
from utils import seed_everything, get_time, save_json, get_tokenizer, save_pickle

timenow = get_time()
cfg = parse_args_train_pl()
seed_everything(cfg.seed)
df = pd.read_pickle(cfg.data_path)
pl_df = pd.read_pickle(cfg.pl_path)
...
for fold in range(s, e):
    ...
    print(f'fold {fold}, n_train {len(train_df)}, n val {len(val_df)}')
    ...
    char_logits = get_char_logits(val_df['pn_history'].values, predictions, tokenizer,
                                  do_fix_offsets=cfg.do_fix_offsets)
